...
## Context
This is a research project. Read more about it from `RESEARCH.md:1-17` (strictly only those lines). 

### Goal
Investigate the possibility / viability of replacing upstream dependencies of a downstream software with an AI-generated bespoke implementation to eliminate inherited vulnerabilities.
...
* The bespoke implementation should not use logic on functions from outside the repository (no external imports to implement the custom PDF.js library)
* The bespoke implementation should be done independent of test files, that is, do not read the contents of `tests/corpus/*.pdf`, only understand and support the test cases. 
...
1. Analysis / Planning Phase - An analysis agent that:
  ...
  pdf renderer to replace pdf.js' functional role in Gogs.
    ...
    - outputs: `.claude/agents-memory/analyzer/analysis.md` that details the implementer phase
  structure, component breakdown, and agent sequence.

2. Implementation Phase (sequential) - multiple implementer agents that:
    - Read `.claude/agents-memory/analyzer/analysis.md` before starting
    - Read the previous agent's handoff at `.claude/agents-memory/handoffs/<agent-name>.md`
    - Implement their assigned phase
    - Write a handoff to `.claude/agents-memory/handoffs/<agent-name>.md` before terminating
...
* The iframe in `templates/repo/view_file.tmpl` should require only a path change
  from `pdfjs-1.4.20` to `custom-pdf-render` to use the replacement
