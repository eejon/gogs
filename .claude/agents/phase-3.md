---
name: phase-3
description: Implements pdf view entry point to interact with rendered pdf.
---
## Context
Read `.claude/agents-memory/analyzer/analysis.md` and `.claude/agents-memory/analyzer/MEMORY.md` before starting.
Read `.claude/agents-memory/handoffs/phase2-renderer.md`.

## Your Task
Implement phase 3 as specified in `analysis.md`, section "Phase 3: Viewer UI and Entry Point".

If the phase has sub-phases, complete only 1 sub-phase, picking up from where the last agent left off. 

## Definition of Done
- All checkboxes in the Phase 3 test section of `analysis.md` must pass.
...
## Handoff
When done, write `.claude/agents-memory/handoffs/phase3-viewer.md` containing:
- What was built and where
- Public interface (classes, key functions, inputs, outputs)
- Assumptions made
- Any known gaps or deferred decisions

This applies to sub-phases as well. When a subphase is complete, append to the same `phase3-viewer.md`.
