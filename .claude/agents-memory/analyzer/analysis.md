...
- BitsPerComponent support: 1, 2, 4, and 8 bpc (1bpc is common in scanned docs; 4bpc in indexed color)
- PNG predictor filters in FlateDecode image streams (Predictor 10–15: None, Sub, Up, Average, Paeth per scanline) — very common in PDFs from LaTeX, LibreOffice, and similar generators
- Common color spaces: DeviceRGB, DeviceGray, DeviceCMYK (convert to RGB for canvas)
- Indexed color spaces (`/Indexed [base hival lookup]`) with palette lookup
- ICCBased color spaces: fall back to device color space based on `/N` (1→Gray, 3→RGB, 4→CMYK)
...
- Internal links (page destinations): navigate within the viewer. Must handle `/GoTo` actions with `/D` destination arrays (e.g., `[pageRef /Fit]`, `[pageRef /XYZ left top zoom]`) and named destinations resolved from the catalog's `/Dests` dictionary or `/Names` → `/Dests` name tree
...
The implementation is split into 5 sequential agents. Each agent reads this analysis document and the previous agent's handoff at `.claude/agents-memory/handoffs/<agent-name>.md`.

```
analysis.md → Phase 1 → handoff → Phase 2a → handoff → Phase 2b → handoff → Phase 3 → handoff → Phase 4
```

**Why Phase 1 stays whole:** The parser components are deeply intertwined — xref tables reference objects, objects contain streams, streams need decompression, the page tree needs object resolution. Splitting would force the second agent to constantly modify the first agent's code. DEFLATE is hard but self-contained algorithmically within the parser.

**Why Phase 2 splits into 2a and 2b:** Phase 2a produces a working renderer for text + paths + colors. Phase 2b adds features that plug in through existing extension points (`Do` operator dispatches to image handling, `sh`/`gs` are new operators, Type3 plugs into font handling). The 2b agent extends rather than rewrites.

| Agent | Scope | Key output |
|-------|-------|------------|
| Phase 1 | Full PDF parser + security filter | `pdf-parser.js`, `pdf-security.js` |
| Phase 2a | Core renderer — tokenizer, graphics state, text, paths, colors, standard 14 fonts, coordinate transforms | `pdf-renderer.js` (core) |
| Phase 2b | Additive features — expanded image decoding, Type3 fonts, shading, ExtGState | Extends `pdf-renderer.js`, may add `pdf-images.js` |
| Phase 3 | Viewer — HTML, JS, CSS, fetch, navigation, zoom, link overlays with `/GoTo` resolution | `viewer.html`, `pdf-viewer.js`, `viewer.css` |
| Phase 4 | Integration — template change, test corpus validation, security audit | Template edit, test results |
...
1. **`js/pdf-parser.js`** — PDF binary parser
   ...
   - Stream decompression: FlateDecode (implement raw inflate — the RFC 1951 DEFLATE algorithm), ASCIIHexDecode, and ASCII85Decode
   - FlateDecode DecodeParms handling: pass through Predictor, Columns, Colors, BitsPerComponent parameters for image stream reconstruction
   - Object reference resolution (`N M R` → resolved object)
   - Page tree traversal: Catalog → Pages → Page objects
   - Named destination extraction from catalog `/Dests` dictionary or `/Names` → `/Dests` name tree
...
**Agent:** `phase-2`
**Scope:** Build the content stream interpreter and canvas rendering engine with core text, path, and color support. Takes parsed page data from Phase 1 and renders it to an HTML5 Canvas.

**Input:** Phase 1's parser output (structured page objects with content streams and resources). Read Phase 1's handoff at `.claude/agents-memory/handoffs/phase-1.md`.
...
1. **`js/pdf-renderer.js`** — Content stream interpreter and canvas renderer
   ...
   - **Basic image rendering** (Phase 2b will expand this):
     - `Do` operator to render XObject images and form XObjects
     - JPEG passthrough (DCTDecode): create `Image()` element from raw bytes
     - FlateDecode images: reconstruct pixel data for 8bpc DeviceRGB/DeviceGray
...
- The renderer architecture must be extensible — Phase 2b will add image format coverage, Type3 fonts, shading, and ExtGState by extending operator dispatch and font/image handling

**Definition of done:**
- Text-only PDFs render with correct positioning and readable text
- Basic images (8bpc JPEG and simple FlateDecode) display at correct position and scale
- Mixed content (text + basic images + vector graphics) renders correctly
...
- The `Do`, font, and operator dispatch points are structured so Phase 2b can extend them without rewriting core logic
...
**Agent:** `phase-2`
**Scope:** Extend the Phase 2a renderer with full image format coverage, Type3 font rendering, shading patterns, and ExtGState support. These are additive features that plug into the existing renderer's extension points.

**Input:** Phase 2a's renderer code and handoff at `.claude/agents-memory/handoffs/phase-2a.md`. Understand the renderer architecture before modifying.

**Components to extend/add:**
1. **Extended image rendering** (expand Phase 2a's basic image support):
   - BitsPerComponent support: 1, 2, 4, and 8 bpc (1bpc is common in scanned docs; 4bpc in indexed color)
   - PNG predictor filters in FlateDecode image streams (Predictor 10–15: None, Sub, Up, Average, Paeth per scanline) — very common in PDFs from LaTeX, LibreOffice, and similar generators
   - Indexed color spaces (`/Indexed [base hival lookup]`) with palette lookup
   - ICCBased color spaces: fall back to device color space based on `/N` (1→Gray, 3→RGB, 4→CMYK)
...
2. **Type3 font rendering** (extend Phase 2a's font handling):
   - Type3 fonts (`/Subtype /Type3`): parse the font dictionary's `/CharProcs` (each entry is a content stream defining a glyph), `/FontMatrix`, and `/FontBBox`
   - Render each glyph by interpreting its CharProc content stream using the same content stream interpreter
   - Type3 glyph streams use `d0`/`d1` operators to declare glyph metrics

3. **Shading and extended graphics state** (new operators):
   - `gs` operator: apply graphics state from ExtGState dictionaries (at minimum: `/ca` and `/CA` for fill/stroke alpha transparency)
   - `sh` operator: render shading patterns — implement axial (Type 2) and radial (Type 3) gradient shading. For shading function types, implement Type 2 (exponential interpolation) and Type 3 (stitching). Render unsupported function types (e.g., Type 4 PostScript calculator) as a flat color fallback using the function's endpoint values

**Output for next phase:**
- Extended `pdf-renderer.js` (or additional module like `pdf-images.js` if separation is cleaner)
- Same API surface as Phase 2a: `renderer.renderPage(pageData, canvas, scale)` → `Promise<void>`

**Definition of done:**
- Images with 1, 2, 4, 8 bpc render correctly
- PNG predictor-filtered FlateDecode images render correctly
- Indexed and ICCBased color space images render correctly
- Image masks and soft masks produce correct transparency
- Type3 font glyphs render by interpreting CharProc content streams
- Axial and radial gradient shading renders
- ExtGState alpha transparency applies correctly
- All Phase 2a tests still pass (no regressions)

**Key implementation notes:**
- Understand Phase 2a's operator dispatch and image rendering path before modifying. Extend, don't rewrite.
- PNG predictors operate per-scanline: each scanline has a filter byte prefix (0=None, 1=Sub, 2=Up, 3=Average, 4=Paeth), then filtered pixel bytes. Reverse the filter to get raw pixel data.
- For Type3 fonts, the content stream interpreter is called recursively — glyph streams are mini content streams. Apply the font's `/FontMatrix` transform before interpreting each glyph stream.
- For shading, the key data is the `/Coords` array (defining the gradient geometry) and the `/Function` (defining the color at each point). Type 2 functions use `f(x) = C0 + x^N * (C1 - C0)`. Type 3 functions stitch together sub-functions over domain intervals.
...
**Input:** Phase 1's parser and Phase 2a/2b's renderer. Read Phase 2b's handoff at `.claude/agents-memory/handoffs/phase-2b.md`.
...
2. **`js/pdf-viewer.js`** — Viewer application logic
   ...
   - **Link annotation rendering**: Overlay clickable regions for link annotations
     ...
     - Internal page links: resolve `/GoTo` actions with `/D` destination arrays (e.g., `[pageRef /Fit]`, `[pageRef /XYZ left top zoom]`) and named destination strings (looked up from catalog) to navigate within the viewer
...
- For link annotations, parse the `/Annots` array on each page, find entries with `/Subtype /Link`, extract the `/Rect` coordinates, and overlay positioned `<a>` elements on top of the canvas. For `/GoTo` actions, resolve the destination to a page index and navigate within the viewer. For `/URI` actions, open externally.
...
│   ├── pdf-renderer.js      # Phase 2a: Core canvas rendering engine
│   ├── pdf-images.js        # Phase 2b: Extended image decoding (optional — may be inlined in renderer)
